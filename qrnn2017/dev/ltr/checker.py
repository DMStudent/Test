...
if __name__ == '__main__':
  ...
  while True:
    ...
    if len(tokens) == 4:
      ...
      hidden_qt = hidden_query + '\t' + hidden_title + '\t' + docid
      if not qt_to_score.has_key(hidden_qt):
        qt_to_score[hidden_qt] = score
      #else:
      #  if (score != qt_to_score[hidden_qt]):
      #    sys.stderr.write('Error: qt-"%s" has different scores, %.6f-%.6f\n'%(hidden_qt,qt_to_score[hidden_qt], score))
