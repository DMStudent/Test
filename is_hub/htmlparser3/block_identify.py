            ...
            if input_node.type and input_node.type == "text":
                ...
                if name:
                    ...
                    if name.find('keyword')!=-1 or name.find('key')!=-1:
                        input_node_hit = True
                        # ("wwj debug tmp find input name hit keyword %s %s", input_node.xpath, input_node.attributes['name'])
                        break
                if _value:
                    if _value.find(u"输入")!=-1 or _value.find(u"关键")!=-1:
                        # ("wwj debug tmp find input value hit keyword %s %s", input_node.xpath, input_node.attributes['value'])
                        input_node_hit = True
                        break
