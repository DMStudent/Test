...
class WebkitStyle(object):
    ...
    def create_webkitstyle_from_str(cls, str):
        ...
        for item in items:
            ...
            if _item[0] == "height":
                height = round(float(_item[1].split('px')[0]))
            if _item[0] == "width":
                width = round(float(_item[1].split('px')[0]))
...
class MainBlock(object):
    ...
    def height(self):
        if not self.blocks:
            return
        top_block = min(self.blocks, key= lambda item:item.webkit_style.top if item.webkit_style else 999999)
        bottom_block = max(self.blocks, key= lambda item:(item.webkit_style.top+item.webkit_style.height) if item.webkit_style else 0)
        #bottom_block = max(self.blocks, key=lambda item: (item.webkit_style.top + item.webkit_style.height))
        if not top_block.webkit_style:
            return
        if not bottom_block.webkit_style:
            return
    ...
    def width(self):
        ...
        left_block = min(self.blocks, key=lambda item: item.webkit_style.left if item.webkit_style else 999999)
        #right_block =  max(self.blocks, key= lambda item:(item.webkit_style.left+item.webkit_style.width))
        right_block = max(self.blocks, key=lambda item: (item.webkit_style.left + item.webkit_style.width) if item.webkit_style else 0)
        if not left_block.webkit_style:
            return
        if not right_block.webkit_style:
            return
