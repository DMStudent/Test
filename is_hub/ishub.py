...
def predict(is_hub, url, body):
    is_hub.get_main_block(url, body)
    if is_hub.main_block is None:
        page_type = "-1.0"
        return page_type, ["none"]
    page_type = is_hub.predict_page_type(url)
    texts = []
    for mblock in is_hub.main_block.blocks:
        for block in mblock.all_text_node_list:
            texts.append(re.sub(u"[？>！\`\~\!\@\#\$\?\^\&\*\%\(\)\=\|\{\}\'\:\;\,\[\]\.\<\>\/\~\\！\？\>]", "", block.text))

    return page_type, texts

def genLabelFromHtml(line):
    line = line.split("\t")
    if len(line) < 2:
        return ""
    else:
        ...
        page_type, texts = predict(is_hub, url, body)
        titlelist = ""
        for text in texts:
            if text.strip():
                titlelist = titlelist + text.encode('utf-8').replace("\n", "").replace("\t", "").replace("\r", "").replace(" ", "").strip() + " "
        titlelist = titlelist[:-1]
        page_type = url + "\t" + str(page_type) + "\t" + titlelist
        return page_type
...
if __name__ == "__main__":
    ...
    url = "http://www.51test.net/jianzhushi/shanghai/moni/"
        #body = line[1]
    body = """"""

    f_result = open("page_type.lst", "w")
    with open("html_result.txt","r") as f:
        for line in f.readlines():
            page_type = genLabelFromHtml(line)
            print page_type
            f_result.write(page_type)
    f_result.close()
