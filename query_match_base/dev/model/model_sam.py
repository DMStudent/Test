...
class Model(ModelBase):
    def model_setup(self):
        ...
        title_enc, att = NASMAttention([query_enc, title_states, title_final, self.title_sent_mask],
                                       [args.max_sent_length, args.num_layers, args.units],
                                       keep_prob=self.keep_prob, is_training=self.is_training)
        query_shape = query_enc.get_shape()
        print("getting query_shape:")
        print(query_shape)

        title_shape = title_enc.get_shape()
        print("getting title_shape:")
        print(title_shape)

        att_shape = att.get_shape()
        print("getting att_shape:")
        print(att_shape)
