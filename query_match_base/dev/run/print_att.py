    ...
    with tf.Graph().as_default(), tf.Session(config=configproto) as sess:
        ...
        for test_file in args.testfnms:
            ...
            for i in range(n_batches):
                ...
                pair_loss, acc, acc01, att, score = \
                    model.run_epoch_att(sess, [q, qm, t, tm, g, False])
                tpair_loss, tacc, tacc01 = tpair_loss + pair_loss, tacc + acc, tacc01 + acc01
                out_str = "%f %f %f" % (pair_loss, acc, acc01)
                #print('att_len=%d'%len(att))
                #print('att[0]_len=%d'%len(att[0]))
                #print('att[0][0]_len=%d'%len(att[0][0]))
                ...
                for j in range(0,len(score)):
                  ...
                  if len(pair_tokens) == 3:
                    score_str1 = pair_tokens[0] + '\t' + pair_tokens[1] + '\t' + pair_tokens[2] + '\n'
                    score_str1 += str(score[j][0]) + '\t' + str(score[j][1]) +'\n'
                    for k in range(30):
                      score_str1 += str(att[j][0][k]) + ' '
                    score_str1 += '\n'
                    for k in range(30):
                      score_str1 += str(att[j][1][k]) + ' '
                    score_str1 += '\n\n'
