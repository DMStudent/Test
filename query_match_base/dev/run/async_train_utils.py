...
def initworker(server, cluster, model):
    with tf.Graph().as_default(), tf.device(tf.train.replica_device_setter(
        ...
        with tf.device("/gpu:{}".format(args.deviceid)):
             vt, vs, _ = model.model_setup()
        args.write_variables(vs)
        # create supervisor
...
def trainLoop(model, sess, dataset):
    ...
    while 1:
        loops += 1
        q, qm, t, tm, g = dataset.get_batch()
        # modified by sam
        #q, qm, t, tm, t_url, g = dataset.get_batch()

        t_time = time.time()
        gstep, loss, pair_loss, regu_loss, acc, acc01, score, _ = \
            model.run_epoch(sess, [q, qm, t, tm, g, True])
        ...
        if loops % args.showperbatch == 0:
            ...
            out_str = "Score: %f - %f, Gap Index %f" % (score[0][0], score[0][1], g[0])
            args.message(out_str, True)
        ...
        if not args.ischief:
            ...
            if new_tik <= tik:
                ...
                if not os.path.exists(ckptfnm):
                    ...
                    #  os.system('sh merge_12k.sh %s %s %s'%(args.output, args.sKeyckptfnm, ckpt_id))
                    #time.sleep(((new_tik + 1) % 20) * 10)
                    #if args.task_index == 3:
                    #  os.system('sh merge_40k.sh %s %s %s'%(args.output, args.sKeyckptfnm, ckpt_id))
