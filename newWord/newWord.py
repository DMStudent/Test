...
s = f.read().decode("utf-8")  # 读取为一个字符串
f.close()
# 定义要去掉的标点字
drop_dict = [u'，', u'\n', u'。', u'、', u'：', u'(', u')', u'[', u']',
                 u'.', u',', u' ', u'\u3000', u'”', u'“', u'？', u'?', u'！',
...
min_support = 2  # 录取词语最低支持度，1代表着随机组合
min_s = 1  # 录取词语最低信息熵，越大说明越有可能独立成词
max_sep = 4  # 候选词语的最大字数
t = []  # 保存结果用。
...
# 保存结果并输出
pd.DataFrame(pd.concat(t[1:])).to_csv('result.txt', header=False, encoding="utf-8")
